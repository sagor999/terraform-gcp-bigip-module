...
resource "google_compute_instance" "f5vm01" {
  ...
  dynamic "network_interface" {
    ...
    content {
      network = network_interface.value.network_id ? network_interface.value.network_id : null
      subnetwork = network_interface.value.subnet_id
      subnetwork_project = network_interface.value.subnet_project_id ? network_interface.value.subnet_project_id : null
